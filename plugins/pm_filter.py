...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        buttons = [[
            ...
            InlineKeyboardButton('🔮 𝗦ᴛᴀᴛᴜs', callback_data='stats')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
        ...
        buttons = [[
            InlineKeyboardButton('🤖 𝗨ᴘᴅᴀᴛᴇs', url='https://t.me/AMD_LinkZz'),
            InlineKeyboardButton('♥️ 🆂🅾🆄🆁🅲🅴', callback_data='source')
        ], [
            InlineKeyboardButton('🏠🅷🅾🅼🅴', callback_data='start'),
...
async def auto_filter(client, msg, spoll=False):
    ...
    if settings["button"]:
        btn = [
            [
                InlineKeyboardButton(
                    text=f"[{get_size(file.file_size)}] {file.file_name}", url=await shorturllink_url(f"https://telegram.me/{temp.U_NAME}?start={pre}_{file.file_id}")
                ),
            ]
            for file in files
        ]
        ...
        btn = [
            [
                InlineKeyboardButton(
                    ...
                    url=await shorturllink_url(f"https://telegram.me/{temp.U_NAME}?start={pre}_{file.file_id}"),
                ),
                InlineKeyboardButton(
                    text=f"{get_size(file.file_size)}",
                    url=await shorturllink_url(f"https://telegram.me/{temp.U_NAME}?start={pre}_{file.file_id}"),
